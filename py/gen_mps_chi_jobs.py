...
JOB = """#!/bin/bash
#SBATCH --job-name=mps{chi}_{comp}
...
#SBATCH --mem={mem}
#SBATCH --time={time}
#SBATCH --output={jobdir}/mps{chi}_{comp}_%j.out
...
echo "[mps chi={chi}] component file = {filestem}.{ext}"
"$JULIA" mps_calc.jl \\
    chi={chi} \\
    cutoff=0.0 \\
    split={split} \\
...
echo "[mps chi={chi}] {filestem} done."
"""
...
def main():
    ...
    # File stem pattern and extension. Jet (.dat):  "jet_{comp}_{timestep}"
    # JHS 1024^3 (.mat):                            "{comp}_t1"  with
    #   --components U,V,W --ext mat
    file_pattern = _parse(args, "--file-pattern", "jet_{comp}_{timestep}")
    ext = _parse(args, "--ext", "dat")
    split = _parse(args, "--split", "1")
    jobdir = os.path.abspath(_parse(args, "--jobdir", f"mps_chi{chi}_jobs"))
    mem = _parse(args, "--mem", "100G")
    cpus = _parse(args, "--cpus", "4")
    ...
    for comp in comps:
        script = JOB.format(
            ...
            filestem=file_pattern.format(comp=comp, timestep=timestep),
            ext=ext,
        )
        path = os.path.join(jobdir, f"job_mps{chi}_{comp}.sh")
    ...
    print("[gen] or run one at a time: "
          f"sbatch {jobdir}/job_mps{chi}_u.sh")
